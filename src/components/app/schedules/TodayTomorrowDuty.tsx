...
import type { DutyOverview, DutyRosterEntry } from "@/modules/schedule-repository/infrastructure/dashboardQueries";
...
function DayCard({ title, date, entries }: { title: string; date: string; entries: DutyRosterEntry[] }) {
  return (
...
export function TodayTomorrowDuty({
  ...
  return (
    <section className="space-y-3" aria-labelledby="today-tomorrow-duty-title">
      {mine.length > 0 && (
        <div role="alert" className="rounded-xl border-2 border-amber-500 bg-amber-50 p-4 text-amber-950 shadow-card-sm dark:bg-amber-950/30 dark:text-amber-100">
          <div className="flex items-start gap-3">
            <Siren className="mt-0.5 h-6 w-6 shrink-0" aria-hidden />
            <div>
              <p className="font-display text-lg font-bold">Você está de serviço {mine.some((entry) => entry.date === overview.today) ? "hoje" : "amanhã"}</p>
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        {schedulesHref && (
          <Link href={schedulesHref} className="inline-flex items-center gap-1 text-sm font-semibold text-primary hover:underline">
